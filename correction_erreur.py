...
def codage_DPCM(x, H, W, step,r):
    ...
    for i in range(H):
        for j in range(W):
            ...
            current_pixel_G = err_G[i, j] +
            current_pixel_B = err_B[i,j] +
    return err_R, err_G, err_B


def my_decodeurDPCM(x, err, H, W):
    img = cv2.imread(x)
    R, G, B = CanauxDeCouleur(x)
    xrec = np.zeros((H, W), dtype=np.uint8)
    previous_pixel = 0

    for i in range(H):
        for j in range(W):
            difference = err[i, j]
            current_pixel = difference + int(r[0]*R[i-1][j]+r[1]*R[i][j-1]+r[2]*G[i-1][j]+r[3]*G[i][j-1]+r[4]*B[i-1][j]+r[5]*B[i][j-1])
            xrec[i, j] = np.clip(current_pixel, 0, 255)  # s'assurer que la valeur est dans [0, 255]
